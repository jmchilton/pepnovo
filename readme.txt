...
4. Quality filtering and Precursor mass correction using PepNovo 

5. Generating queries for MS-Blast (see msblast_guide.pdf)
...
PepNovo now has the capability to perform filtering/charge selection and parent mass correction. Currently the only model supporting this capability is CID_IT_TRYP. The quality score is in the range 0-1, where 0 means the spectrum is likely to be noise (without a peptide signal) and 1 means the spectrum is likely to contain a peptide signal. The quality score is generated using logistic regression models trained on samples of good spectra (that had peptide ids) and bad spectra (that received a very low score in a database search). Each spectrum was converted to a feature vector with features that capture various aspects that can help distinguish between the two classes of spectra.  For example, some of the features used involve the distribution of peaks, the number of large peaks, the length of amino acid ladders (sequences of peaks with an amino acid mass difference between them).

A useful option option when running PepNovo is:

-min_filter_prob <x=0-1.0>

Which filters out spectra with a quality probability less than x (e.g., x=0.1).  This can reduce the amount of noisy low scoring results.


5. Generating queries for MS-Blast
----------------------------------

See the file msblast_guide.pdf
