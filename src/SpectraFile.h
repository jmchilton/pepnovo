...
public:
	...
	int	scanFile(const char *filePath, int generationIdx, int fileIndexInList, 
...
private:
	...
	int	scanDtaFile(const char *filePath, int generationIdx, int fileIndexInList, const Config* config, bool removeDuplicates = true);
	int	scanMgfFile(const char *filePath, int generationIdx, int fileIndexInList, const Config* config, bool removeDuplicates = true);
	int	scanMzxmlFile(const char *filePath, int generationIdx, int fileIndexInList, const Config* config, bool removeDuplicates = true);
	int	scanDatFile(const char *filePath, int generationIdx, int fileIndexInList, const Config* config, bool indOverwriteDatLocation = false);
	int	scanPklFile(const char *filePath, int fileIndexInList, Config* config);
	int	scanMS2File(const char *filePath, int fileIndexInList, Config* config);
	...
	int	readMs2PeakList(FILE* stream, const SingleSpectrumHeader* header, Peak* peaks, Config* config);
