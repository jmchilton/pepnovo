...
public:
	...
	{
		pmcsqs.train_pmc_rank_models(&config_,fm,charge);
	}
	...
	void train_sqs(const FileManager& pos_fm, const char *neg_list, int specific_charge, vector< vector<float> > *weights=NULL)
	{
		pmcsqs.train_sqs_models(&config_, pos_fm,neg_list, specific_charge, weights);
	}
	...
	{
		return pmcsqs.get_best_mz_charge(&config_,bs,mz1,charge1,prob1,mz2,charge2,prob2,all_res);
	}
	...
	bool get_ind_pmcsqs_was_intialized() const { return (pmcsqs.getIndInitializedPmcr() &&
...
//	}
	...
	void read_rank_models(const char *name, bool silent_ind = false, bool use_large_db_model = false);
	...
	void predict_fragmentation(const char* input_file, size_t num_peaks=20);
