										...
										mass_t maxMz)
{
	...
	for (size_t i=0; i<paths.size(); i++)
	{
		...
		if ( sscanf(buffer,"#RANGE\t%f\t%f",&minExcludeMz, &maxExcludeMz) != 2)
		{
			...
			ScanEntry se;
			if (sscanf(buffer,"%d\t%d\t%d\t%f", &se.generationIdx, &se.fileIdx, &se.scan, &se.mz)>=3)
			{
				scans_[se] = i;
				numExclusions++;
			}
...
struct ScanStruct {
	ScanStruct() : generationIdx(-1), fileIdx(-1), scan(-1), mz(0.0), charge(0) {}
	bool operator< (const ScanStruct& rhs) const
	{
		return ( generationIdx <  rhs.generationIdx ||
				(generationIdx == rhs.generationIdx && fileIdx <  rhs.fileIdx) ||
				(generationIdx == rhs.generationIdx && fileIdx == rhs.fileIdx && scan < rhs.scan));
	}
...
void makeScanListFromClust(const char* list, const char* scanFilePath)
{
	...
	for (size_t i=0; i<paths.size(); i++)
	{
		...
		while ( ! clustStream.eof())
		{
			...
			for (size_t j=0; j<size; j++)
			{
				...
				istringstream iss(buffer);
				ScanStruct s;
				iss >> s.generationIdx >> s.fileIdx >> s.scan >> s.mz >> s.charge;
				if (s.generationIdx<0 || s.fileIdx<0 || s.scan<0 || s.mz<=0.0)
				{
					cout << "Bad line: " << buffer << endl;
					cout << "File: " << paths[i] << endl;
					exit(1);
				}
				assert(s.generationIdx>=0 && s.fileIdx>=0 && s.scan>=0 && s.mz>0.0);
	...
	for (size_t i=0; i<allScans.size(); i++)
	{
		if (i>0 && allScans[i].scan == allScans[i-1].scan && allScans[i].fileIdx == allScans[i-1].fileIdx
				&& allScans[i].generationIdx == allScans[i-1].generationIdx)
		{
			numSame++;
			if (numSame<10)
				cout << "SAME: " << allScans[i].generationIdx << "\t" << allScans[i].fileIdx
					 << "\t" << allScans[i].scan << "\t" << allScans[i].mz << endl;
		}
		ofs << allScans[i].generationIdx << "\t" << allScans[i].fileIdx << "\t"
