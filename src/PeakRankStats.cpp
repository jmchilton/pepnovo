...
//	fms[3].init_from_list_file(config,"C:\\Work\\msms5\\NewScore\\Shew_98_3_unique_mgf_list.txt");
	...
	{
		char list[256];
		sprintf(list,"C:\\Work\\msms5\\NewScore\\%s_98_%d_unique_mgf_list.txt",name,c);
		fms[c].init_from_list_file(config,list);
		make_min_max_peak_mass_reports(fms[c],config,c,name);
	}
	...
	for (c=1; c<=3; c++)
	{
		cout << endl << "CHARGE " << c << endl << endl;
		make_proton_mobility_summary(fms[c], config);
	}
}
