...
public:
	...
	void read_denovo_rank_scorer_model(const char *path, string type_string, bool silent_ind = false, bool use_large_db_model = false);
	...
	bool get_ind_part_model_was_initialized(int charge, int size_idx) const { 
		return (charge < dnv_part_models.size() &&
				size_idx < dnv_part_models[charge].size() &&
			    dnv_part_models[charge][size_idx] && 
				dnv_part_models[charge][size_idx]->ind_was_initialized); }
	...
	void scoreCompleteSequences(const vector<PeptideSolution>& peptideSols,
								AnnotatedSpectrum& as,
								vector<score_pair>& scorePairs,
								int forcedSizeIndex=-1) const;
...
private:
	...
	void fillCompletePeptideRankboostSample(const PeptideSolution& sol,
							   AnnotatedSpectrum& as,
							   const vector<PmcSqsChargeRes>& res,
							   RankBoostSample& rbs,
							   int size_idx=-1) const;
...
void benchmark_ranking_on_full_denovo(void *model, 
									  char *mgf_test_file, 
									  int max_num_spectra,
									  int num_solutions,
									  char *report_path,
...
void make_ranking_examples(void *model,
						   char *mgf_test_file);
...
void make_peak_hist_for_obs_rank(void *model, 
								 char *benchmark_file, 
								 int obs_rank);
