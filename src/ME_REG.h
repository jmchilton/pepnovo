...
public:
	...
	float get_sum_exp(const ME_Regression_Sample& sam) const
	{
		...
		assert(sam.f_vals.back().f_idx<f_weights.size());
		float sum_exp=0;
		for (size_t i=0; i<sam.f_vals.size(); i++)
		{
	...
	void set_num_features(int n) { num_features = n; f_weights.resize(n,0.0); };
	...
	const vector<double>& getWeights() const { return f_weights; }
