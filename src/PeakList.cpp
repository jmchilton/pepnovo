										 ...
										 const SingleSpectrumHeader* header)
{
		  ...
		  peaks_[0].mass != header->getFirstPeakMass() ) )
	{
		...
		if (numPeaksRead != header->getOriginalNumPeaks())
			cout << "Num peaks read " << numPeaksRead << ", expecting " <<  header->getOriginalNumPeaks() << endl;
...
void PeakList::createIndexArray(vector<int>& indexArray, mass_t externalMaxMass) const
{
	const mass_t maxPeakMass = (peaks_[numPeaks_ -1].mass > externalMaxMass ? peaks_[numPeaks_ -1].mass  : externalMaxMass);
...
//	cout << "TI: " << header_->getPrecursorIntensity() << endl;
...
void PeakList::printSomePeaks() const
{
	...
	cout << "num peaks: " << numPeaks_ << endl << fixed << setprecision(3);
...
size_t	PeakList::outputToMgfBuffer(char* buffer, const SingleSpectrumHeader* newHeader) const
{
	...
	if (totalPeakIntensity_ > totalNormalizedPeakIntensity_)
	{
		...
		float weight = totalPeakIntensity_ / totalNormalizedPeakIntensity_;
		for (size_t i=0; i<numPeaks_; i++)
		{
...
// outputs an mgf spectrum BEGIN IONS ... END IONS to a buffer
// returns the number of bytes written (assumes there is enough room)
void	PeakList::outputToMgfStream(ostream& os, const SingleSpectrumHeader* newHeader) const
{
	os << "BEGIN IONS" << endl;
	char buffer[1024];
	if (newHeader)
	{
		newHeader->writeHeaderToMgfBuffer(buffer);
	}
	else
		header_->writeHeaderToMgfBuffer(buffer);

	os << buffer;

	// write peaks
	os << fixed << setprecision(NUM_SIG_DIGITS);
	...
	if (totalPeakIntensity_ > totalNormalizedPeakIntensity_)
	{
		...
		for (size_t i=0; i<numPeaks_; i++)
		{
			os << peaks_[i].mass << " " << peaks_[i].intensity*weight;
			if (peaks_[i].charge>0)
				os << " " << peaks_[i].charge;
			os << endl;
		}
		...
		for (size_t i=0; i<numPeaks_; i++)
		{
			os << peaks_[i].mass << " " << peaks_[i].intensity;
			if (peaks_[i].charge>0)
				os << " " << peaks_[i].charge;
			os << endl;
		}
