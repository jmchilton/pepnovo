...
public:
	...
	SingleSpectrumHeader() : mOverZ_(NEG_INF), originalNumPeaks_(0), originalPmWith19_(NEG_INF), pmWith19_(MIN_FLOAT),
						charge_(0), fileType_(0), msLevel_(2), scanNumber_(MIN_INT), indexInFile_(-1), positionInFile_(0), 
						clusterSize_(1), spectraFileIndexInList_(MIN_INT), archiveFileIndex_(MIN_INT), 
						generationIndex_(MIN_INT), retentionTime_(0), precursorIntensity_(0),  
						firstPeakMass_(-1.0), sqs_(NEG_INF), title_(""), peptideStr_("") {}
	...
	int		getArchiveFileIndex()  const { return archiveFileIndex_; }
	void	setArchiveFileIndex(int i)   { archiveFileIndex_ = i; }

	int		getGenerationIndex() const { return generationIndex_; }
	void    setGenerationIndex(int i)  { generationIndex_ = i; }
	void    setGenerationIndex(int i) const  { generationIndex_ = i; }
	...
	void    intializeFromSingleSpectrumFile(void* ssf_ptr);
...
private:
	...
	int		archiveFileIndex_; /*! The index of the dat file in the list of archive files. */

	mutable int generationIndex_; /*! For clustering jobs, this can be set to different values so we can tell 
