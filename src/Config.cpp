...
void Config::computeSizeThresholds(const SpectraAggregator& sa)
{
	...
	int charge;
	for (charge=sa.getMinSpectrumCharge(); charge<=sa.getMaxSepctrumCharge(); charge++)
	...
	// report:
	cout << endl << "Partitioning according to charge/size:" << endl;
	cout         << "--------------------------------------" << endl;
	for (charge = sa.getMinSpectrumCharge(); charge<=sa.getMaxSepctrumCharge(); charge++)
	{
		...
		for (i=0; i<massThresholdsForSizes_[charge].size(); i++)
			cout <<  i << "\t" << (i>0 ? massThresholdsForSizes_[charge][i-1] : 0) << " to "
				 << massThresholdsForSizes_[charge][i] << endl;
		cout << endl;
	}
							...
							float minFragmentProbability )
{
	...
	for (charge=1; charge<=sa.getMaxSepctrumCharge(); charge++)
	{
	...
	//	select_frag_combos(fm,&config_,c,max_num_combos);

		selectFragmentCombinations(sa, this, charge, 3);
	}

	string fragments_file = get_resource_dir() + "/" + get_model_name() + "_fragments.txt";
	ofstream os(fragments_file.c_str(),ios::out);
	print_fragments(os);
	set_fragments_file(fragments_file);
	os.close();

	string regional_fragment_sets_file = get_resource_dir() + "/" + get_model_name() + "_fragment_sets.txt";
...
void Config::set_digest_type(int type)
{
	...
	if (digest_type == GLUC_DIGEST)
	{
		c_term_digest_aas.push_back(Asp);
		c_term_digest_aas.push_back(Glu);
		return;
	}
...
//		os << "#CONF RESOURCE_DIR " << resource_dir << endl;
	...
	os << "#CONF DIGEST_TYPE " << digest_type << " (" << NON_SPECIFIC_DIGEST << " - non specific, " <<
		 TRYPSIN_DIGEST << " - trypsin " << GLUC_DIGEST << " - GluC)" << endl;
