...
int SpectraFile::scanFile(const char *filePath, int generationIdx, int fileIndexInList, 
						  const Config* config, bool removeDuplicates, bool overwriteExisitngLocations)
{
	...
	{
		scanMgfFile(filePath, generationIdx, fileIndexInList, config, removeDuplicates);
	}
	else if (fileType_ == IFT_MZXML)
	{
		scanMzxmlFile(filePath, generationIdx, fileIndexInList, config);
	}
	else if (fileType_ == IFT_DAT)
	{
		scanDatFile(filePath, generationIdx, fileIndexInList, config, overwriteExisitngLocations);
	}
	else if (fileType_ == IFT_DTA)
	{
		scanDtaFile((unzippedFilePath_.length() ? unzippedFilePath_.c_str() : filePath), 
					 generationIdx, fileIndexInList, config, removeDuplicates);
	}
	else
...
int	SpectraFile::scanDtaFile(const char *filePath, int generationIdx, int fileIndexInList, const Config* config,
							 bool removeDuplicates)
{
	...
	while (true)
	{
		...
		ssh.setGenerationIndex(generationIdx);
...
int SpectraFile::scanMgfFile(const char* filePath, int generationIdx, int fileIndexInList, const Config* config,
							 bool removeDuplicates)
{
	FILE* mgfStream=fopen(filePath,"r");
	if (! mgfStream)
	{
...
************************************************************************************/
int SpectraFile::scanMzxmlFile(const char* filePath, int generationIdx, int fileIndexInList, const Config* config,
							   bool removeDuplicates)
{
...
********************************************************************************/
int	SpectraFile::scanDatFile(const char *filePath, int generationIdx, int fileIndexInList, 
								 ...
								 Peak* peaks, const Config* config) const
{
	...
	// check that all peaks have consecutive masses
	bool orderedOk=true;
	for (size_t i=1; i<peakCount; i++)
	...
	if (! orderedOk)
	{
		sort(&peaks[0],&peaks[peakCount]);
		const_cast<SingleSpectrumHeader*>(header)->setFirstPeakMass(peaks[0].mass);
	}
								   ...
								   const Config* config) const
{
	...
	const size_t bytesToRead = 256 + header->getOriginalNumPeaks()* 12;
	...
	for (size_t i=0; i<peakCount; i++)
	{
		...
		if (peaks[peakIdx].intensity>0.0)
			peakIdx++;
	}
