...
void DatFile::peekAtStats(const char* path)
{
	...
	if (! stream_)
		error("Couldn't open file for reading: ",path);
										...
										size_t  endDatStatIdx )
{
	...
	if (endDatStatIdx == MAX_SIZE_T)
		endDatStatIdx = datStats.size();

	assert(startDatStatIdx<=endDatStatIdx && endDatStatIdx<datStats.size());
	assert(datStats[startDatStatIdx].fileIdx == fileIdx_ &&
		   datStats[endDatStatIdx].fileIdx   == fileIdx_);
	...
	while (numSpectraReadFromDat<numSpectraToRead)
	{
		...
		assert( datStats[datStatsIdx].fileIdx == fileIdx_ );
		...
		header.setArchiveFileIndex(archiveFileIndex_); // this is the file number in the dat list
		...
		if (foundError)
		{
			...
			cout << "Archive index: " << header.getArchiveFileIndex() << endl;
