										  ...
										  bool only_basic_score)
{
	...
	// check that the index array is large enough
	if (pm_with_19 > spectrum->getMaximalPeakMassToConsider() )
	{
		spectrum->setMaximalPeakMassToConsider(pm_with_19 + 11.0);
		spectrum->init_index_array();
	}
...
void PrmGraph::create_nodes_for_peptide(const Peptide& pep)
{
	...
	const int orginialSourceCharge = source_spectrum->getCharge();
	source_spectrum->setCharge(charge);
	...
	source_spectrum->setCharge(orginialSourceCharge);
}
