...
public:
	Spectrum() : originalPmWith19_(-1.0),	mOverZ_(-1.),
				 ...
				 scanNumber_(-1),		   retentionTime_(-1),
	...
	void		setMaximalPeakMassToConsider(mass_t m) { maximalPeakMassToConsider_ = m; }

	intensity_t get_total_original_intensity() const { return totalPeakIntensity_; }
	...
	bool read_from_dta(const Config* _config, const char *dta_name, 
					   int charge = -1, bool verbose =false);

	bool read_from_MGF_stream(const Config*_config, FILE *stream, int charge = 0, 
					   bool verbose = false);

	bool read_from_peak_arrays(const Config* _config, Peptide& _pep, mass_t _pm_with_19, 
		int _charge, int _num_peaks, mass_t *_masses, intensity_t *_intensities);

	bool init_from_QCPeaks(const Config* _config, void *QCPeaks, int num_peaks, void *ssf,
...
protected:
	...
	intensity_t	totalPeakIntensity_;// sum of all peak intensities (after normalization)
	...
	int sizeIndex_;
	const Config* config_; // The config used to read this Spectrum
	string title_;
	Peptide peptide_;
