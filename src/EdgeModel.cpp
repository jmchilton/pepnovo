...
void EdgeModel::write_edge_models(const char *model_name)
{
	...
	const string file_path = config_->get_resource_dir() + "/"  + string(model_name) + "_edge_model.txt";
	...
	for (i=0; i<this->regional_edge_models.size(); i++)
	{
		...
		for (j=0; j<regional_edge_models[i].size(); j++)
		{
			...
			for (k=0; k<regional_edge_models[i][j].size(); k++)
			{
				...
				{
					string path = config_->get_resource_dir() + "/" + make_regional_name(model_name,i,j,k);
...
{
	config_ = config;
	const string file_path = config->get_resource_dir() + "/" + string(model_name) + "_edge_model.txt";	
...
/*	const vector< vector< mass_t > >& size_thresholds = config->get_size_thresholds();
	...
	for (c=0; c<size_thresholds.size(); c++)
	{
		...
		for (s=0; s<regional_edge_models[c].size(); s++)
		{
			...
			{
				string path = config_->get_resource_dir() + "/" + make_regional_name(model_name,c,s,r);
