							   ...
							   int specific_size_idx)
{
	...
	//			cout << sc << "\t(" << all_ssf.size() << ")" << endl;
				...
				const float spectrum_quality = (pmcsqs_res.size()>charges[0] ? pmcsqs_res[charges[0]].sqs_prob : 0.0);
		...
		//		cout << sc << "\t" << rank_corr;
			...
			// write models
			char fname[128];
			sprintf(fname,"%s_CSP_%d_%d_%d.txt",config->get_model_name().c_str(),tag_length, charge,size_idx);
...
bool CumulativeSeqProbModel::calc_cumulative_seq_probs(
