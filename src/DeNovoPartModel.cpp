							...
							RankBoostSample& rbs) const
{
	...
	vector<int> sol_aas = sol.pep.get_amino_acids();

	// convert amino acids to non-modified if they are larger than translation table
	// this needs to be done because the version running on CCMS site uses a different mechanism for declaring
	// PTMs. Ideally they should all be added to aa_translations
	for (size_t i=0; i<sol_aas.size(); i++)
		sol_aas[i]=org_aa[sol_aas[i]];
	...
	if (path.positions.size()>3)
	{
		...
		//	int span_cat = comp_assigner->get_aa_category(2,span_aas, false, false);
			const int span_cat = comp_assigner->get_aa_category(2,&sol_aas[idx], false, false);
