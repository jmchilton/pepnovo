...
bool StrongFragmentModel::read_model(const Config* config, istream& is, bool silent_ind)
{
	...
	if (inten_model.getWeights().size()<SI_NUM_FEATURES)
	{
		inten_model.set_num_features(SI_NUM_FEATURES);
		cout << endl << " resize SI " << endl;
	}

	if (no_inten_model.getWeights().size()<SNI_NUM_FEATURES)
	{
		no_inten_model.set_num_features(SNI_NUM_FEATURES);
		cout << endl << " resize SNI " << endl;
	}
