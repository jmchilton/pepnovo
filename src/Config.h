...
public:
	...
	Config() : indWasInitialized_(false), resource_dir("Models"), min_exclude_range(9999999), max_exclude_range(NEG_INF) {};
	...
	int get_num_sizes(int charge) const 
	{	
		if (charge>=regionalFragmentSets_.size())
			return -1;

		return regionalFragmentSets_[charge].size(); 
	}

	int get_num_regions(int charge, int size_idx) const 
	{ 
		if (charge>=regionalFragmentSets_.size() ||
			size_idx >= regionalFragmentSets_[charge].size())
			return -1;
		return regionalFragmentSets_[charge][size_idx].size(); 
	}
...
private:
	...
	int		    digest_type; // 0 - nothing , 1 Trypsin, 2 - GluC
