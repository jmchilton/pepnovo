...
bool PeptideCompAssigner:: read_table_to_vector(char *file_path, int naa, vector<int>& vec)
{
...
bool PeptideCompAssigner::read_and_init_from_tables(Config *_config, const char *name)
{
	config = _config;
	...
	for (i=1; i<=MAX_COMP_LEN; i++)
	{
		...
		if (! read_table_to_vector(file_path,i,start_assigns[i]))
			return false;

		sprintf(file_path,"%s/%s_end_freq_%d.txt",config->get_resource_dir().c_str(),name,i);
		if (! read_table_to_vector(file_path,i,end_assigns[i]))
			return false;

		sprintf(file_path,"%s/%s_mid_freq_%d.txt",config->get_resource_dir().c_str(),name,i);
		if ( ! read_table_to_vector(file_path,i,mid_assigns[i]))
			return false;
	}
...
int PeptideCompAssigner::get_aa_category(int num_aa, const int *aas, bool n_term, bool c_term) const
{
	if (! this->was_initialized)
	{
		...
		cout << "You might need to use an existing composition assigner." << endl;
		cout << "Try adding the line: \"#COMP_ASSIGNER  LTQ_COMP/IT_TRYP\" to the main model file (model_name.txt)" << endl;
		exit(1);
	}
