								 ...
								 char *tag_suffix)
{
	...
	//		continue;
		...
		model.selectPrecursorMassesAndCharges(config, s, pms_with_19, charges, &pmcsqs_res);
	//	model.select_pms_and_charges(config,bs,pms_with_19,charges,&pmcsqs_res);
		...
		{
			cout << setprecision(3) << " \tSQS: " <<  pmcsqs_res[charges[0]].sqs_prob;
		...
		if (sc % 200 == 0)
		{
			...
			cout << "done " << sc+1 << "/" << all_ssf.size() << "  " << curr_time - start_time << " secs.,  "
	...
	for (f_idx=0; f_idx<list_vector.size(); f_idx++)
	{
		...
		for (sc=0; sc<all_ssf.size(); sc++)
		{
			...
			if (alt_pms_with_19.size()>0)
			{
				...
				generate_tags(prm_ptrs,&model,bs,&s, num_tags, main_length,
							  alt_pms_with_19, alt_charges, alt_solutions, false, alt_solutions.size());
				for (int j=0; j<alt_solutions.size(); j++)
					solutions.push_back(alt_solutions[j]);
			}
					...
					int num_test_cases)
{
	...
	//	s.print_spectrum();
		...
		if (pmcsqs_res.size()>charges[0])
		{
			...
			cout << sc << "\t" << num_peaks << "\t" << setprecision(2) << -1.0 << "\t" <<
				ssf->peptide.as_string(config) << "  (" << ssf->peptide.get_mass_with_19() - s.get_org_pm_with_19() << ")" << endl;
									 ...
									 ostream& out_stream)
{
	...
	for (f=0; f<list_vector.size(); f++) 
	{
		...
		for (sc=0; sc<sl.getNumHeaders(); sc++)
		{
			...
			header->printStats(config, out_stream, false);
			AnnotatedSpectrum as;
			if (! as.readSpectrum(sa, header))
			{
				out_stream << "#Problem reading spectrum..." << endl;
				continue;
			}

			SingleSpectrumHeader* nonConstHeader = const_cast<SingleSpectrumHeader*>(header);
			nonConstHeader->setSpectraFileIndexInList(f);
		
			
			if (as.getNumPeaks()<5)
			{
				out_stream << endl << "# too few peaks..." << endl;
				continue;
			}
			...
			{
				out_stream << endl << "# Charge " << as.getCharge() << " not supported yet..." << endl << endl;
				continue;
			}
			...
			if (1)
			{
				...
				if (max_pm>0.0 && pms_with_19.size()>0 && pms_with_19[0] > max_pm)
				{
					out_stream << endl << "# Precursor mass above threshold: " << pms_with_19[0] << endl << endl;
					continue;
				}
				...
				{
					if (&pmcsqs_res)
						out_stream << endl << "# low quality, skipping: " << pmcsqs_res[charges[0]].sqs_prob << endl << endl;
					continue;
				}
				...
				if (pms_with_19[0]<100)
				{
					...
					out_stream << endl << "# Could not process spectrum..." << endl << endl;
					continue;
				}
			...
			else 
			{
				...
				if (output_cumulative_seq_probs)
				{
					...
					goodProbs = csp_model->calc_cumulative_seq_probs(first_sol_charge, first_sol_size_idx, 
						spectrum_quality, score_idx_pairs, solutions); 
				}


				for (size_t i=0; i<solutions.size() && i<num_solutions; i++) 
				{
					const int idx = (perform_rerank ? score_idx_pairs[i].idx : i);
								  ...
								  bool prm_norm)
{
	...
	for (f=0; f<list_vector.size(); f++) 
	{
		...
		for (sc=0; sc<sl.getNumHeaders(); sc++)
		{
			...
			header->printStats(config);
			AnnotatedSpectrum as;
			if (! as.readSpectrum(sa, header))
			{
				cout << "#Problem reading spectrum..." << endl;
				continue;
			}
			...
			model.selectPrecursorMassesAndCharges(config, as, pms_with_19, charges);
			if (pms_with_19.size()<=0)
			{
			...
			if (model.get_config()->get_num_sizes(charges[0])<0)
			{
				cout << "# Charge not supported: " << charges[0] << "..." << endl;
				continue;
			}

			cout << "Charge= " << charges[0] << "  [M+H] = " << pms_with_19[0] << endl;
			print_prm_graph_scores(&model,&as,pms_with_19[0],charges[0], prm_norm);
		}
	}
}


void perform_pmcsqs_on_list_of_files(AllScoreModels& model, 
									 const vector<string>& list_vector,
									 int file_start_idx,
									 ostream& out_stream)
{
	...
	for (f=0; f<list_vector.size(); f++) 
	{
		...
		for (sc=0; sc<sl.getNumHeaders(); sc++)
		{
			...
			if (pmcsqs_res.size()>charges[0])
			{
				out_stream << "SQS: " << pmcsqs_res[charges[0]].sqs_prob;
			}
			else
				out_stream << "SQS: " << " -";
			out_stream << "\tCharge: " << charges[0] << "\t[M+H]: " << pms_with_19[0] << endl;

			if (charges.size()>1)
			{
				if (pmcsqs_res.size()>charges[1])
				{
					out_stream << "SQS: " << pmcsqs_res[charges[1]].sqs_prob;
				}
				else
					out_stream << "SQS: " << " -";
				out_stream << "\tCharge: " << charges[1] << "\t[M+H]: " << pms_with_19[1] << endl;
			}
			out_stream << endl;
		}
	}
					   ...
					   int target_num_spectra, char *out_mgf_name)
{
	...
	fs.select_all_files(fm);
	fs.remove_spectra_with_PTMs();
	fs.randomly_reduce_ssfs(target_num_spectra);
	
	ofstream ofs(out_mgf_name);
	if (! ofs.good())
	{
		cout << "Error: couldn't open file for writing: " << out_mgf_name << endl;
		exit(1);
	}
	...
	cout << "wrote " << all_ssf.size() << " spectra to " << out_mgf_name << endl;
	ofs.close();
}
