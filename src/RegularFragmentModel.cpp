...
bool RegularFragmentModel::read_model(const Config* config, istream& is, bool silent_ind)
{
	...
	if (inten_model.getWeights().size()<RI_NUM_FEATURES)
	{
		inten_model.set_num_features(RI_NUM_FEATURES);
		cout << endl << " resize RI " << endl;
	}

	if (no_inten_model.getWeights().size()<RNI_NUM_FEATURES)
	{
		no_inten_model.set_num_features(RNI_NUM_FEATURES);
	//	cout << endl << " resize RNI " << endl;
	}
