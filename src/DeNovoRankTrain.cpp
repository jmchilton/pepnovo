	...
	if (ratio_db_cross>0)
	{
		...
		if (num_train_db_cross>0) 
		{
			...
			for (i=0; i<num_train_db_cross; i++)
			{
				...
				//	samples[phi_support[bad_idx].idx1].rank_in_group != 1) // cross db with strongest bad db hit
					bad_idx = (int)(num_train_pairs * myRandom());
		...
		if (num_test_db_cross>0)
		{
			...
			for (i=0; i<num_test_db_cross; i++)
			{
				...
				while (bad_idx<0 || phi_support[bad_idx].tag != SOL_INCORRECT_DB )
				//	samples[phi_support[bad_idx].idx1].rank_in_group != 1) // cross db with strongest bad db hit
					bad_idx = (int)(org_support_size * myRandom());
...
void PeptideRankScorer::test_model(char *test_mgf, int max_num_test_cases) const
{
	...
	for (i=0; i<all_ssf.size(); i++)
	{
		...
		prm.create_graph_from_spectrum(allScoreModels,&s,mass_with_19,ssf->charge);
		...
		generate_denovo_solutions(prm_ptr, allScoreModels,&s,true,mass_with_19,2,2000,
		...
		for (s_idx=0; s_idx<sol_paths.size(); s_idx++)
		{
			...
			pep_sol.pep.parseFromString(config,sol_paths[s_idx].seq_str);
