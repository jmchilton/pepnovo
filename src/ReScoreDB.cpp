...
struct InspectResultsLine {
	...
	InspectResultsLine() : SpectrumFile(std::string()), scan(-1), Annotation(std::string()), Protein(std::string()),
							Charge(0), MQScore(-999.0), Score(-999.0), Length(0), TotalPRMScore(0.0), MedianPRMScore(0.0),
							FractionY(0.0), FractionB(0.0), Intensity(0.0), NTT(0), p_value(1.0), F_Score(-999.0),
							DeltaScore(0.0), DeltaScoreOther(0.0), RecordNumber(0), SpecFilePos(0),
							PrecursorMz(0.0), PrecursorError(999.0), orgRank(-1), aaBefore(-1), aaAfter(-1) {}
...
void ScanCandidateSet::output_to_stream(ostream& os, int num_lines) const
{
	...
	for (i=0; i<this->results.size(); i++)
	{
		...
		os << results[i].SpecFilePos << "\t" << results[i].PrecursorMz << "\t" << results[i].PrecursorError << endl;
